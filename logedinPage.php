...
if (isset($_SESSION['user_id'])) {
    ...
    // --- YOUR ORIGINAL CODE (REMAINS) ---
    // Count pending requests where the logged-in user is the Helper
    $sql_count = "SELECT COUNT(*) AS total FROM service_requests 
                  WHERE helperID = $current_user AND status = 'pending'";
    $res_count = mysqli_query($conn, $sql_count);
    $data_count = mysqli_fetch_assoc($res_count);
    $helper_notifs = $data_count['total']; // Store this in a temporary variable

    // --- ADD THIS NEW SECTION BELOW IT ---
    // Count status updates where the logged-in user is the Needer
    $sql_needer = "SELECT COUNT(*) AS total FROM service_requests 
                   WHERE neederID = $current_user AND status != 'pending'";
    $res_needer = mysqli_query($conn, $sql_needer);
    $data_needer = mysqli_fetch_assoc($res_needer);
    $needer_notifs = $data_needer['total'];

    // Combine both for the final notification count
    $notifications = $helper_notifs + $needer_notifs;
...
<body>
    <div class="app-container">
        ...
        <nav>
            <ul>
               ...
               <a href="requests.php" style="text-decoration: none; position: relative;">
              <span style="font-size: 24px;">🔔</span>
         <?php if ($notifications > 0): ?>
            <span class="bell-badge"><?php echo $notifications; ?></span>
         <?php endif; ?>
                </a>
                </li>
                <li type="button"><a href="profile.php" >profile</a>
    ...
    <div class="pic">
        <a href="taskType.php?category=Physical Effort">
        ...
        <a href="taskType.php?category=Babysitting">
            <div class="service-box">
        ...
        <a href="taskType.php?category=House Cleaning">
            <div class="service-box">
        ...
        <a href="taskType.php?category=Patient Care">
            <div class="service-box">
        ...
        <a href="taskType.php?category=Pet Care">
            <div class="service-box">
